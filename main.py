...
parser.register_validator(lambda p: spectral.norm.validate_mode(p.mode), 'Wrong spectral norm parameter')
...
class Main(object):
    def __init__(self, args):
        ...
        self.opt_d = torch.optim.Adam(self.discriminator.parameters(), betas=(.5, .99), lr=self.args.lrd)
        self.opt_g = torch.optim.Adam(self.generator.parameters(), betas=(.5, .99), lr=self.args.lrg)
    ...
    def train(self):
        ...
        for i in range(1, self.args.iters + 1):
            ...
            if self.args.dataset.lower() == 'cifar10' and self.args.eval_every != -1 and i % self.args.eval_every == 0:
                self.log_evaluate(i)
    ...
    def log_evaluate(self, it):
        ...
        with (self.logs / 'inception_score.txt').open('a') as f:
            f.write('{:d},{},{}'.format(it, *inception_score))
...
if __name__ == '__main__':
    ...
    with args.safe_experiment:
        main = Main(args)
        sys.stdout = spectral.logging.TeeOutput(
            sys.stdout, main.logs / 'stdout'
        )
        sys.stderr = spectral.logging.TeeOutput(
            sys.stderr, main.logs / 'stderr'
        )
        print(main.gan)
        main.train()
